...
export function ContactModal({ children }: { children: React.ReactNode }) {
    ...
    return (
        <Dialog>
            ...
            <DialogContent showCloseButton={false} className="sm:max-w-5xl p-0 bg-background border-white/10 text-foreground overflow-hidden shadow-2xl block rounded-3xl">

                {/* Custom Close Button */}
                <DialogClose className="absolute top-6 right-6 z-20 p-2 rounded-full bg-white/5 hover:bg-white/10 transition-colors text-foreground focus:outline-none focus:ring-0">
                    <X className="w-6 h-6" />
                </DialogClose>
                ...
                <div className="flex flex-col md:flex-row h-full md:min-h-[600px] max-h-[90vh]">
                    ...
                    <div className="w-full md:w-[40%] bg-black/20 p-8 md:p-12 flex flex-col justify-between relative overflow-hidden backdrop-blur-md border-r border-white/5">

                        {/* Background Effects - Subtle Violet/Gold */}
                        <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-secondary/20 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
                        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-primary/20 rounded-full blur-[80px] translate-y-1/2 -translate-x-1/2 pointer-events-none" />

                        <div className="relative z-10">
                            <span className="text-primary text-xs font-bold tracking-[0.2em] uppercase mb-4 block">
                                Contacto Directo
                            </span>
                            <DialogTitle className="font-heading text-3xl md:text-5xl font-medium text-foreground mb-6 leading-tight">
                                Hablemos de <br />
                                <span className="font-serif italic text-muted-foreground">
                                    Excelencia.
                                </span>
                            </DialogTitle>
                            <DialogDescription className="text-muted-foreground text-lg leading-relaxed font-light">
